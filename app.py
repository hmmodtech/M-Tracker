# YOUR FULL, ORIGINAL APP.PY CODE WITH THE TWO REQUIRED FIXES
...
# --- FIX #1: This line is corrected to explicitly define how to serve static files. ---
...
# --- ALL YOUR ORIGINAL CODE IS BELOW AND UNCHANGED ---
...
ADMIN_USER = {"id": "u_admin", "username": "aabujami", "email": "hmmodtech@gmail.com", "email2": "aabujami@pt.acfspain.org", "name": "Ahmed Abu Jami", "password": "801165226", "role": "admin", "created": "2024-01-01T00:00:00Z", "isBuiltIn": True, "isSuperAdmin": True}
ADMIN_USER2 = {"id": "u_admin2", "username": "aaljojo", "email": "aaljojo@pt.acfspain.org", "email2": "", "name": "Ashraf Al Jojo", "password": "3030", "role": "admin", "created": "2024-01-01T00:00:00Z", "isBuiltIn": True, "isSuperAdmin": False}
...
def _load_users():
    ...
    if has_admin: users = [{**u, **ADMIN_USER} if u.get('id') == 'u_admin' else u for u in users]
    else: users.insert(0, dict(ADMIN_USER))
    ...
    if not has_admin2:
        ...
        for u in users:
            if u.get('id') == 'u_admin2': u['isBuiltIn'] = True; u['isSuperAdmin'] = False
    return users
...
def api_get_users(): return jsonify(_load_users())

@app.route('/api/users', methods=['POST'])
def api_save_users():
    try:
        data = request.get_json(force=True, silent=True)
        if not isinstance(data, list): return jsonify({'error': 'Expected a list'}), 400
        has_admin = any(u.get('id') == 'u_admin' for u in data)
        if not has_admin: data.insert(0, dict(ADMIN_USER))
        else: data = [{**u, **ADMIN_USER} if u.get('id') == 'u_admin' else u for u in data]
...
def api_get_channels(): return jsonify(db.get_channels())
...
def api_add_channel():
    ...
    try: scraper.scrape_channel(username)
    except Exception as e: logger.warning(f'Initial scrape failed for {username}: {e}')
...
def api_get_bookmarks(): return jsonify(db.get_bookmarks())

@app.route('/api/bookmarks', methods=['POST'])
def api_add_bookmark():
    data = request.get_json(force=True, silent=True) or {}
    mid = data.get('message_id')
    if not mid: return jsonify({'error': 'message_id required'}), 400
    db.add_bookmark(mid)
...
def api_get_keywords(): return jsonify(db.get_keywords())
...
def api_geocode_reverse():
    ...
    except (TypeError, ValueError): return jsonify({'error': 'lat and lon required'}), 400
...
def api_export():
    ...
    if mode == 'bookmarks': return jsonify(db.get_bookmarks())
...
# --- FIX #2: The conflicting route that caused the 404 error is now deleted. ---
# @app.route('/<path:filename>')
# def serve_static(filename):
#     return send_from_directory('static', filename)

def run_scrape():
    try: scraper.scrape_all()
    except Exception as e: logger.error(f'Scheduled scrape error: {e}')
