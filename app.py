...
def api_register_device():
    ...
    user_id = session.get('user_id', '')
    db.register_device_token(token, user_id)
    return jsonify({'ok': True})
...
def run_scrape():
    try:
        before_count = db.count_messages()
        scraper.scrape_all()
        after_count = db.count_messages()
        added = after_count - before_count
        if added > 0:
            fresh_critical = db.get_messages(limit=added, critical_only=True)
            if fresh_critical:
                # Step 1: collapse near-duplicates reported by different
                # channels within this same batch (one representative each).
                stories = db.group_similar_messages(fresh_critical)

                # Step 2: drop any story that's too similar to something
                # already notified within the last DUP_WINDOW_HOURS — this
                # covers the same event being picked up by another channel
                # in a *later* scrape cycle, not just this one.
                recent_sets = db.get_recent_notified_token_sets()
                new_stories = []
                for s in stories:
                    toks = set(db.tokenize_for_dedup(s.get('text', '')))
                    if len(toks) < db.DUP_MIN_SHARED_TOKENS:
                        new_stories.append(s)  # too short to compare reliably, don't block it
                        continue
                    is_dup = any(db.jaccard(toks, rs) >= db.DUP_JACCARD_THRESHOLD for rs in recent_sets)
                    if not is_dup:
                        new_stories.append(s)
                        recent_sets.append(toks)
                        db.record_notified_story(toks)

                if new_stories:
                    tokens = db.get_all_device_tokens()
                    if tokens:
                        if len(new_stories) == 1:
                            m = new_stories[0]
                            title = "M-TRACKER Critical Alert"
                            body = (m.get('summary') or m.get('text') or '')[:180]
                        else:
                            title = f"M-TRACKER: {len(new_stories)} Critical Alerts"
                            body = (new_stories[0].get('summary') or new_stories[0].get('text') or '')[:150]
                        result = push.send_to_tokens(tokens, title, body, critical=True)
                        if result.get('invalid_tokens'):
                            db.remove_device_tokens(result['invalid_tokens'])
                        logger.info(f"[push] Sent critical alert to {result.get('sent',0)} device(s), "
                                    f"{len(new_stories)} unique stor{'y' if len(new_stories)==1 else 'ies'} "
                                    f"(of {len(fresh_critical)} raw critical message(s))")

                db.prune_old_notified_stories()
    except Exception as e:
